...
source('utils_inference_conditional_extinction.R')
...
## Restrict analysis to chains that ended
df_res <- df_res %>% filter(tot_cluster_size < curr_max_cluster_size_sim)
...
plt_Rs <- df_inference %>% 
  ...
  scale_y_continuous(name = 'Rs estimate',
