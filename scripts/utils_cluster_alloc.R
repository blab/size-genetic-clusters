...
## Allocating clusters of identical sequences based on a distance matrix
get_identical_clusters_from_dist_mat_with_dates <- function(dist_mat, metadata_seq){
  ...
  ## Number of sequences without any other identical sequences
  df_singletons <- dist_mat %>%  melt() %>%  as_tibble() %>% 
    rename(seq_1 = Var1, seq_2 = Var2, n_mut = value) %>% 
    filter(n_mut == 0) %>% 
    group_by(seq_1) %>% summarise(n_identical = n()) %>% 
    filter(n_identical == 1) 
  n_singletons <- df_singletons %>% nrow()
  vec_name_singletons <- df_singletons$seq_1
  
  if(n_singletons == nrow(dist_mat)){
    g_identical <- NULL
    df_size_distrib <- tibble(cluster_size = 1, count = n_singletons)
    df_clusters_full <- tibble(seq = vec_name_singletons,
                               clique_id = 1:n_singletons)
    
    df_cluster_info <- df_clusters_full %>% 
      left_join(metadata_seq, by = c('seq' = 'strain')) %>% 
      group_by(clique_id) %>% 
      summarise(date_first_cluster_detection = min(date),
                cluster_size = n())
  ...
  df_clusters <- tibble(seq = V(g_identical)$name,
                        clique_id = V(g_identical)$clique_id)
  if(n_singletons > 0){
    df_clusters_full <- bind_rows(df_clusters,
                                  tibble(seq = vec_name_singletons,
                                         clique_id = (max(df_clusters$clique_id) + 1):(max(df_clusters$clique_id) + n_singletons)))
